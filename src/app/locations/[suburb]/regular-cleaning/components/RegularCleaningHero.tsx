...
interface RegularCleaningHeroProps {
  locationData: LocationData;
  suburbSlug: string;
}

export function RegularCleaningHero({ locationData, suburbSlug }: RegularCleaningHeroProps) {
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="space-y-6 lg:space-y-8">
            ...
            <div className="space-y-3 lg:space-y-4">
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-bold text-gray-900 leading-tight">
                Regular House Cleaning in{" "}
                <span className="text-[#1E3D8F]">{capitalizeFirstLetter(locationData.name)}</span>
              </h1>
              
              <p className="text-base sm:text-lg lg:text-xl text-gray-600 leading-relaxed">
                Keep your {capitalizeFirstLetter(locationData.name)} home consistently clean with trusted local cleaners. Same cleaner each visit, flexible times, and simple online booking backed by our satisfaction guarantee.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <span className="text-sm sm:text-base text-gray-700 font-medium">Weekly or Fortnightly</span>
              </div>
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <span className="text-sm sm:text-base text-gray-700 font-medium">60-Second Booking</span>
              </div>
            </div>

            {/* CTA Buttons */}
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <Link href={`/book?selectedServices=Regular%20Cleaning&location=${encodeURIComponent(locationData.name)}`}>
                <Button 
                  size="lg" 
                ...
                >
                  Book Regular Cleaning in {capitalizeFirstLetter(locationData.name)}
                </Button>
              </Link>
          ...
          <div className="relative">
            <div className="relative h-[350px] sm:h-[400px] lg:h-[500px] xl:h-[600px] rounded-2xl overflow-hidden shadow-2xl">
              <Image
                src="/general-cleaning.png"
                alt={`Professional house cleaning service in ${capitalizeFirstLetter(locationData.name)}`}
                fill
