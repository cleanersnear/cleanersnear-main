...
// Local type to remove external dependency
interface Suburb {
  name: string;
  postcode: string;
  region: string;
...
export default function LocationSearch() {
  ...
  // Using booking store directly to persist selected suburb/postcode
  ...
  const handleSuburbSelect = (suburb: Suburb) => {
    ...
    // No storage - just validation for service area
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 md:px-0">
      {/* Search Container - Responsive: column on mobile, row on md+ */}
      <div className="relative">
        <div className="flex flex-col md:flex-row gap-2 md:gap-3">
          ...
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-4 md:px-4 py-3 md:py-3 pl-14 md:pl-12 border-2 border-gray-200 rounded-md md:rounded-md
            ...
            <div className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2">
              <MapPin className="w-4 h-4 md:w-5 md:h-5 text-[#1E3D8F] animate-bounce" />
            </div>
          </div>

          {/* Action Button */}
          <button
            className={`group w-full md:w-auto px-4 md:px-5 py-3 md:py-3 rounded-md md:rounded-md font-semibold text-base md:text-base transition-all duration-300 
              flex items-center justify-center gap-2 whitespace-nowrap
              bg-[#1E3D8F] text-white shadow-lg
            `}
            onClick={handleCheckAvailability}
          >
            <span className={`${selectedSuburb ? 'inline-flex items-center gap-2 wobble' : 'inline-flex items-center gap-2'}`}>
              {selectedSuburb ? 'See Instant Pricing — Let\'s goo' : 'Get Free Quote'}
              <ArrowRight className={`w-4 h-4 ml-1 ${selectedSuburb ? 'arrow-bounce' : ''}`} />
            </span>
          </button>
        </div>
        ...
        {/* Styled dropdown list (no Google API, no framer-motion) */}
        {showSuggestions && searchQuery.length > 0 && (
          <div
            className="absolute z-50 left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden transition-all duration-200"
          >
